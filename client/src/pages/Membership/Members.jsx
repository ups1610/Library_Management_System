...
import UpdateMember from "../../components/Membership/UpdateMember";
...
function Members() {
  ...
  const updateMembershipStatus = async (members) => {
    ...
    for (const member of members) {
      try {
        const response = await MemberAction.getMemberMembership(
          member.memberId,
          token
        );
    ...
    if (members) {
      ...
      for (const member of members) {
        try {
          ...
          updatedStatusMap[member.memberId] = response.data
            ? "Active"
            : "Not Active";
        } catch (error) {
          ...
          <table className="w-full divide-y-2 divide-gray-200 bg-white text-sm">
            <thead className="divide-y-2 divide-gray-200 bg-slate-900">
              <tr>
                ...
                <th className="whitespace-nowrap px-4 py-2 font-medium text-white md:hidden">
                  Member ID
                </th>
                <th className="whitespace-nowrap px-4 py-2 font-medium text-white">
                  First Name
              ...
              {loading ? (
                ...
                members.map((member, index) => (
                  <tr key={index}>
                    ...
                    <td className="text-left px-6 py-4 whitespace-nowrap md:hidden">
                    ...
                    <td className="text-left px-2 py-4 whitespace-nowrap">
                      <button
                        className={`rounded-full px-3 py-1 text-sm ${
                          membershipStatus[member.memberId] === "Active"
                    ...
                    <td className="text-right px-2 py-4 whitespace-nowrap">
                      <div className="inline-flex rounded-lg">
                        <button
                          ...
                          className="inline-flex mx-1 items-center gap-2 rounded-md px-4 py-2 text-sm bg-blue-500 text-white hover:bg-blue-700 focus:relative"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                        ...
                        <Link
                          to={`${member.memberId}/info`}
                          className="inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm bg-green-500 text-white mx-1 hover:bg-green-700 focus:relative"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                        ...
                        <button
                          onClick={(e) => deleteMember(e, member.memberId)}
                          className="inline-flex items-center mx-1 gap-2 rounded-md px-4 py-2 text-sm bg-red-500 hover:bg-red-700 text-white focus:relative"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                        ...
                        <button
                          onClick={() => handleAddMembership(member.memberId)}
                          className="inline-flex items-center mx-1 gap-2 rounded-md px-4 py-2 text-sm  border-2 focus:relative"
                        >
                          <MdOutlineLibraryAdd /> Membership
                        </button>
                      </div>
                    </td>
                  </tr>
