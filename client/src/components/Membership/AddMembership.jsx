...
function AddMembership({ selectedMember, onClose }) {
    ...
    const [amount,setMembershipPlanAmount]=useState(0);
    useEffect(() => {
    ...
    const getAllMembershipPlans = () => {
        setLoading(true);
        MemberAction.getAllMemberPlans(token)
            .then((response) => {
                if (response.success) {
    ...
    const handleChange = (e) => {
        ...
        if (name === 'membershipPlanId') {
            ...
            setMembershipPlanAmount( selectedPlan.price );
            }
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        MemberAction.addMembership(selectedMember.memberId, membershipData, token)
            .then((response) => {
                if (response.success) {
                    if(membershipData.modeOfPayment==='Cash'){
                        toast.success("Membership Added Successfully");
                    }else{
                        console.log(response)
                        MemberAction.requestPayment(amount,response.data.memberName,response.data.membershipId,"Membership",token);

                        toast.success("Payment Link Generated. Ask Member to pay membership.");
                    }
            ...
            <div className="bg-white p-8 rounded-lg shadow-md">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="flex flex-col mb-4">
                        ...
                        >
                            ...
                            {membershipPlans.map(plan => (
                                <option key={plan.id} value={JSON.stringify(plan)}>{plan.planName} ₹ {plan.price}</option>
                            ))}
                        </select>
                    </div>
                    <div className="flex justify-end mt-4">
                        ...
                        <button type="submit" className="px-4 py-2 bg-black text-white font-semibold rounded" disabled={loading}>
